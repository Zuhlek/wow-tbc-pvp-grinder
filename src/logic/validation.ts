...
/**
 * Validate config for manual mode (no end date required).
 */
export function validateConfigManual(config: AppConfig): ValidationResult {
  const errors: string[] = [];
  ...
  // Manual games per day validation
  if (config.manualGamesPerDay < 0) {
    errors.push('manualGamesPerDay must be >= 0');
  }
